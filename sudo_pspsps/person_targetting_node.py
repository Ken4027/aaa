...
class PersonTargetingNode(Node):
    ...
    # ── Target-lock hysteresis (R4 follow-up) ─────────────────────────────────
    # "Closest person" already picks who to track (R4), but frame-to-frame
    # noise or two people standing at nearly the same distance can otherwise
    # make that choice flicker between candidates every frame. These settings
    # make the lock "sticky": once we're on someone, a different candidate
    # only steals the lock if they are clearly closer AND enough time has
    # passed since the last switch. See "未解決の問題" in the v2 guide.
    TARGET_SWITCH_MARGIN_MM    = 300   # depth mode: new person must be this much nearer (mm)
    TARGET_SWITCH_MARGIN_FRAC  = 0.20  # area mode: new bbox must be this fraction larger
    TARGET_SWITCH_MIN_HOLD_SEC = 1.5   # minimum time between target switches
    TARGET_MATCH_MAX_PX        = 250   # max centre-drift (px) still counted as "same person"
    ...
    def __init__(self):
        ...
        # ── Target-lock state (R4 hysteresis) ─────────────────────────────────
        # Which bbox centre / distance-metric we're currently locked onto, and
        # when we last allowed the lock to switch to a different person. Reset
        # whenever we drop back to SEARCHING (see _enter_searching).
        self._locked_center     = None   # (cx, cy) in pixels
        self._locked_metric     = None   # depth median (mm) or bbox area (px^2)
    ...
    def _image_callback(self, msg: Image):
        ...
        now_sec = self.get_clock().now().nanoseconds / 1e9
        xmin, ymin, xmax, ymax = self._closest_person(persons, now_sec)
    ...
    def _enter_searching(self, now_sec: float):
        ...
        # Drop the target-lock hysteresis state too — whoever we track next
        # after re-acquiring should be evaluated fresh, not measured against
        # a person we may not even be able to see anymore.
        self._locked_center       = None
        self._locked_metric       = None
        self._search_glance_idx   = 0
    ...
    def _closest_person(self, persons, now_sec: float):
        """
        R4: pick who to track when multiple people are in frame — always the
        closest one, never "react to everyone" or an arbitrary pick.

        Depth is preferred when available (more physically meaningful than
        bbox size); falls back to bbox area otherwise. Either way the raw
        "who is closest right now" choice is passed through
        _apply_lock_hysteresis() so a momentary noisy reading, or two people
        standing at nearly the same distance, doesn't cause the tracked
        target to flicker between them every frame.
        """
        if self._latest_depth is not None:
            return self._closest_by_depth(persons, now_sec)
        return self._closest_by_area(persons, now_sec)

    def _closest_by_depth(self, persons, now_sec: float):
        depth  = self._latest_depth
        dh, dw = depth.shape[:2]
        shrink = self.DEPTH_BBOX_SHRINK

        # (bbox, median_depth_mm) for every person with a valid depth read.
        candidates = []
        for (xmin, ymin, xmax, ymax) in persons:
            ...
            if valid.size == 0:
                continue
            candidates.append(((xmin, ymin, xmax, ymax), float(np.median(valid))))

        if not candidates:
            # Nobody had a usable depth reading this frame — fall back to
            # area-based selection (still goes through the same hysteresis).
            return self._closest_by_area(persons, now_sec)

        best_bbox, best_metric = min(candidates, key=lambda c: c[1])
        return self._apply_lock_hysteresis(
            candidates, best_bbox, best_metric, now_sec, lower_is_better=True,
        )

    def _closest_by_area(self, persons, now_sec: float):
        candidates = [
            (bbox, float((bbox[2] - bbox[0]) * (bbox[3] - bbox[1])))
            for bbox in persons
        ]
        best_bbox, best_metric = max(candidates, key=lambda c: c[1])
        return self._apply_lock_hysteresis(
            candidates, best_bbox, best_metric, now_sec, lower_is_better=False,
        )

    def _apply_lock_hysteresis(self, candidates, best_bbox, best_metric,
                                now_sec: float, lower_is_better: bool):
        """
        Decide whether to keep tracking whoever we're currently locked onto,
        or hand the lock to `best_bbox` (this frame's raw "closest" pick).

        Without this, two people at nearly equal distance — or noisy
        per-frame depth/area estimates — can make the tracked target swap
        back and forth every frame, which reads as unstable rather than
        attentive. The lock only moves to a new person when:
          1. we can no longer find the previously-locked person nearby
             (they left frame / were replaced), or
          2. the new candidate clearly beats them by TARGET_SWITCH_MARGIN_*
             AND at least TARGET_SWITCH_MIN_HOLD_SEC has passed since the
             last switch.
        """
        def centre(bbox):
            return ((bbox[0] + bbox[2]) / 2.0, (bbox[1] + bbox[3]) / 2.0)

        # First lock ever (or just came out of SEARCHING): take the best
        # candidate outright, nothing to be sticky about yet.
        if self._locked_center is None:
            chosen = best_bbox
            self._last_switch_time = now_sec
            self._locked_center = centre(chosen)
            self._locked_metric = best_metric
            return chosen

        # Find whichever candidate is spatially closest to where the locked
        # person was last seen — this is how we "re-identify" them frame to
        # frame without any persistent ID from MediaPipe.
        locked_bbox, locked_dist = min(
            (
                (bbox, math.hypot(centre(bbox)[0] - self._locked_center[0],
                                   centre(bbox)[1] - self._locked_center[1]))
                for bbox, _ in candidates
            ),
            key=lambda c: c[1],
        )
        still_tracking_locked_person = locked_dist <= self.TARGET_MATCH_MAX_PX

        if not still_tracking_locked_person:
            # Whoever we were locked onto is no longer findable nearby —
            # e.g. they walked out of frame. Re-lock onto the best candidate.
            chosen = best_bbox
            self._last_switch_time = now_sec
            self.get_logger().info(
                "Locked person no longer found — re-locking onto closest candidate."
            )
        elif locked_bbox == best_bbox:
            # The locked person is still the closest — nothing to decide.
            chosen = locked_bbox
        else:
            locked_metric = next(
                m for bbox, m in candidates if bbox == locked_bbox
            )
            hold_elapsed = (now_sec - self._last_switch_time) >= self.TARGET_SWITCH_MIN_HOLD_SEC

            if lower_is_better:
                clearly_better = (locked_metric - best_metric) >= self.TARGET_SWITCH_MARGIN_MM
            else:
                clearly_better = best_metric >= locked_metric * (1.0 + self.TARGET_SWITCH_MARGIN_FRAC)

            if hold_elapsed and clearly_better:
                chosen = best_bbox
                self._last_switch_time = now_sec
                self.get_logger().info(
                    "Target lock switched — new candidate is clearly closer."
                )
            else:
                # Someone else reads as marginally closer this frame, but not
                # by enough of a margin (or not for long enough) to justify
                # switching — keep tracking the same person.
                chosen = locked_bbox

        self._locked_center = centre(chosen)
        self._locked_metric = next(m for bbox, m in candidates if bbox == chosen)
        return chosen
